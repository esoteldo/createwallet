...
export function highloadWalletV3ConfigToCell(config: HighloadWalletV3Config): Cell {
    return beginCell()
        ...
        .storeUint(config.subwalletId, 32) // seqno, not used in wallet state
...
export class HighloadWalletV3 implements Contract {
    ...
    async getSegno(provider: ContractProvider): Promise<number> {
        const res = (await provider.get('get_segno', [])).stack;
        return res.readNumber();
    }
